...
function ForgotPassword() {
  ...
  const handleVerifyEmail = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post("/auth/verify-email", { email: email.trim().toLowerCase() });
      setStep(2);
    } catch (err) {
  ...
  const inputClass = "w-full pl-9 pr-10 py-3 bg-[#1e1e2e] border border-[#3a3a4a] rounded-xl text-white text-sm placeholder:text-gray-600 focus:outline-none focus:border-[#FFC0C1] focus:ring-1 focus:ring-[#FFC0C1]/30 transition-colors";
  ...
  return (
    <div className="min-h-screen bg-[#1e1e2e] flex items-center justify-center px-4 py-10">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[400px] h-[400px] bg-[#FFC0C1]/5 rounded-full blur-3xl" />
        <div className="absolute bottom-1/4 right-1/4 w-[300px] h-[300px] bg-[#97C9DB]/5 rounded-full blur-3xl" />
      </div>
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-[#FFC0C1]/10 border border-[#FFC0C1]/30 mb-4">
            <svg className="w-8 h-8 text-[#FFC0C1]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8}
                d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
            </svg>
          </div>
          <h1 className="text-3xl sm:text-4xl font-bold text-[#FFC0C1] tracking-widest">
            Reset Password
          </h1>
        ...
        <div className="flex items-center gap-3 mb-6 px-2">
          <div className="flex items-center gap-2 flex-1">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-colors
              ${step >= 1 ? "bg-[#FFC0C1] border-[#FFC0C1] text-[#1e1e2e]" : "border-[#3a3a4a] text-gray-600"}`}>
              {step > 1 ? (
            ...
            <span className={`text-xs font-medium ${step >= 1 ? "text-[#FFC0C1]" : "text-gray-600"}`}>Verify Email</span>
          </div>
          <div className={`h-px flex-1 ${step >= 2 ? "bg-[#FFC0C1]/50" : "bg-[#3a3a4a]"} transition-colors`} />
          <div className="flex items-center gap-2 flex-1 justify-end">
            <span className={`text-xs font-medium ${step >= 2 ? "text-[#FFC0C1]" : "text-gray-600"}`}>New Password</span>
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-colors
              ${step >= 2 ? "bg-[#FFC0C1] border-[#FFC0C1] text-[#1e1e2e]" : "border-[#3a3a4a] text-gray-600"}`}>
              2
            </div>
          </div>
        </div>
        ...
        {/* Card */}
        <div className="bg-[#282828] border border-[#3a3a4a] rounded-2xl p-6 sm:p-8 shadow-2xl">
          ...
          {step === 1 && (
            <form onSubmit={handleVerifyEmail} className="flex flex-col gap-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={email} onChange={e => { setEmail(e.target.value); setError(""); }}
                    ...
                    className="w-full pl-9 pr-4 py-3 bg-[#1e1e2e] border border-[#3a3a4a] rounded-xl text-white text-sm placeholder:text-gray-600 focus:outline-none focus:border-[#FFC0C1] focus:ring-1 focus:ring-[#FFC0C1]/30 transition-colors" />
                </div>
              </div>
              ...
              <button type="submit" disabled={loading}
                className={`w-full py-3 rounded-lg font-semibold text-sm tracking-wide transition flex items-center justify-center gap-2 border-2 border-[#1e1e2e]
                  ${loading
            ...
            <form onSubmit={handleResetPassword} className="flex flex-col gap-5">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type={showConfirm ? "text" : "password"} value={confirmPassword}
                    ...
                    placeholder="Re-enter new password" required className={inputClass} />
              ...
              <div className="flex gap-3">
                <button type="button"
                  ...
                  className="flex-1 py-3 rounded-lg font-semibold text-sm border-2 border-[#3a3a4a] text-gray-400 hover:bg-white/5 hover:text-white transition">
                  Back
                </button>
                <button type="submit" disabled={loading}
                  className={`flex-1 py-3 rounded-lg font-semibold text-sm tracking-wide transition flex items-center justify-center gap-2 border-2 border-[#1e1e2e]
                    ${loading
                      ? "bg-gray-700 text-gray-400 cursor-not-allowed border-gray-700"
                      : "bg-[#FFC0C1] text-[#1e1e2e] hover:bg-[#ffb0b1] shadow-lg"}`}>
                  {loading ? (
          ...
          <div className="mt-6 text-center">
            <p className="text-gray-500 text-sm">
              ...
              <button onClick={() => navigate("/")} className="text-[#FFC0C1] hover:text-[#ffb0b1] font-medium transition">
                Sign In
              </button>
            </p>
          </div>
